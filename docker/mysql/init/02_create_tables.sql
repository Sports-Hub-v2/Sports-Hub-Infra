...
CREATE TABLE accounts (
    ...
    userid VARCHAR(50) UNIQUE COMMENT 'User ID for login',
...
CREATE TABLE profiles (
    ...
    is_ex_player BOOLEAN DEFAULT FALSE COMMENT 'Has professional experience',
...
CREATE TABLE teams (
    ...
    captain_profile_id BIGINT COMMENT 'Captain profile ID',
    ...
    FOREIGN KEY (captain_profile_id) REFERENCES profiles(id) ON DELETE SET NULL,
    INDEX idx_region (region),
    INDEX idx_skill_level (skill_level),
    INDEX idx_status (status)
...
CREATE TABLE team_rivals (
    team_id BIGINT NOT NULL,
    rival_team_id BIGINT NOT NULL,

    rivalry_level VARCHAR(20) DEFAULT 'NORMAL' COMMENT 'NORMAL, INTENSE, FRIENDLY',
    rivalry_desc VARCHAR(255) COMMENT 'Description of rivalry',

    created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,

    PRIMARY KEY (team_id, rival_team_id),
    FOREIGN KEY (team_id) REFERENCES teams(id) ON DELETE CASCADE,
    FOREIGN KEY (rival_team_id) REFERENCES teams(id) ON DELETE CASCADE,
    INDEX idx_rival_team (rival_team_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Team rivalries';
